...
- **Always in motion**: Frames must *enter and exit in motion*. Do not freeze before the cut; micro‑motion continues through the last frame.
...
- **Transitions never pause content**: Clips continue moving *through* transitions. The transition is the flow, not a booking stop.
...
```json
{
  "timeline": {
    "tracks": [
      { "clips": [ { "asset": { "type": "video", "src": "A.mp4" }, "start": 0, "length": 7, "transition": { "out": "fade" } } ]},
      { "clips": [ { "asset": { "type": "video", "src": "B.mp4" }, "start": 6, "length": 5 } ]},
      { "clips": [ { "asset": { "type": "video", "src": "overlays/soft-dust.mov" }, "start": 6, "length": 1.5 } ]}
    ]
  }
}
```
...
```json
{
  "timeline": {
    "tracks": [
      ...
      {
        "clips": [
          { "asset": { "type": "video", "src": "B.mp4" }, "start": 3, "length": 4 }
        ]
      }
    ]
  }
      ...
      { "clips": [
        {
          ...
          "start": 0, "length": 6,
          "offset": {
            "x": [ { "from": 0, "to": 0.02, "start": 5, "length": 1, "interpolation": "bezier", "easing": "easeInOutSine" } ]
          },
          "opacity": [
...
- **Music**: Soft drone *in A minor*. Swell then soften across each piece to imply a harmonic journey.
...
- Clip B: either start fully visible underneath *or* tween 0 → 1 for symmetrical dissolve.
