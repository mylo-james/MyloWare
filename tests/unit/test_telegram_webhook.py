...
def test_webhook_processes_message(monkeypatch):
    ...
    monkeypatch.setattr("api.routes.telegram.send_telegram_message", AsyncMock(return_value=True))

    fake_agent = MagicMock()
    fake_agent.create_turn.return_value = SimpleNamespace(
        completion_message=SimpleNamespace(content="ok")
    )
    monkeypatch.setattr("api.routes.telegram.create_supervisor_agent", lambda client: fake_agent)
