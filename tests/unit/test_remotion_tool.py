...
def test_remotion_tool_name_and_schema():
    ...
    assert "composition_code" in params
    assert "clips" in params
    assert params["clips"].required is True


def test_remotion_tool_fake_mode():
    with patch("tools.remotion.settings") as mock_settings:
        mock_settings.use_fake_providers = True
        mock_settings.remotion_service_url = "http://localhost:3001"
        mock_settings.webhook_base_url = "http://example.com"
...
def test_remotion_tool_submits_payload():
    captured = {}
...
def test_remotion_tool_requires_clips():
    with patch("tools.remotion.settings") as mock_settings:
        mock_settings.use_fake_providers = True
        mock_settings.remotion_service_url = "http://localhost:3001"
        mock_settings.webhook_base_url = "http://example.com"
...
def test_remotion_tool_raises_http_error():
    with (
        patch("tools.remotion.settings") as mock_settings,
        patch("httpx.Client") as mock_client_cls,
    ):
