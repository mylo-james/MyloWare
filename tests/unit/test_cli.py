...
def test_demo_aismr_interactive(monkeypatch):
    ...
    monkeypatch.setattr("cli.main.get_session", fake_session)
    monkeypatch.setattr("cli.main.get_sync_client", lambda: object())
    monkeypatch.setattr("cli.main.run_workflow", fake_run_workflow)
    monkeypatch.setattr(
        "workflows.langgraph.workflow.continue_after_ideation", fake_continue_after_ideation
    )
    monkeypatch.setattr(
        "workflows.langgraph.workflow.continue_after_publish_approval",
...
def test_kb_setup(monkeypatch):
    ...
    monkeypatch.setattr("cli.main.get_sync_client", lambda: object())
    monkeypatch.setattr(
        "knowledge.loader.load_documents_with_manifest", fake_load_documents_with_manifest
    )
    monkeypatch.setattr("knowledge.setup.setup_project_knowledge", fake_setup_project_knowledge)
...
def test_stack_shields_retrieve(monkeypatch):
    ...
    monkeypatch.setattr("cli.main.get_sync_client", lambda: fake_client)
