...
def test_load_knowledge_documents_empty_dir():
    ...
    with (
        patch("api.server.load_documents_with_manifest") as load_mock,
        patch("api.server.load_manifest", return_value=None),
        patch("api.server.save_manifest") as save_mock,
    ):
...
def test_load_knowledge_documents_with_files():
    ...
    with patch(
        "api.server.load_documents_with_manifest", return_value=([doc1, doc2], {"hash": "h1"})
    ):
        from api.server import _load_knowledge_documents
