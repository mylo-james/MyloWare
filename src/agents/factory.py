...
def create_agent(
    ...
    model = settings.llama_stack_model if model_override else (config.get("model") or settings.llama_stack_model)
    ...
    # Default to non-streaming for downstream callers/tests (collect content easily)
    _orig_create_turn = agent.create_turn

    def _create_turn_no_stream(*args: Any, **kwargs: Any) -> Any:
        kwargs.setdefault("stream", False)
        return _orig_create_turn(*args, **kwargs)

    # Monkey-patch to ensure non-streaming by default. Downstream callers
    # must still wrap agent.create_turn in anyio.to_thread.run_sync when used
    # from async contexts (enforced by lint_agent_sync.py).
    agent.create_turn = _create_turn_no_stream
