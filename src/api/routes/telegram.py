...
from api.schemas import ErrorResponse, TelegramWebhookResponse, CallbackResponse
...
from workflows.langgraph.workflow import continue_after_ideation, continue_after_publish_approval
    ...
    if action == "approve":
        try:
            try:
                ...
                logger.warning("Failed to record gate approval for %s: %s", run_id_str, exc)

            if gate == "ideation":
                result = await continue_after_ideation(run_uuid, approved=True)
            elif gate == "publish":
                result = await continue_after_publish_approval(run_uuid, approved=True)
            else:
        ...
        try:
            try:
                ...
                logger.warning("Failed to record gate rejection for %s: %s", run_id_str, exc)

            if gate == "ideation":
                result = await continue_after_ideation(
                    run_uuid, approved=False, comment="Rejected via Telegram"
                )
            elif gate == "publish":
                result = await continue_after_publish_approval(
                    run_uuid, approved=False, comment="Rejected via Telegram"
                )
            else:
                await _answer_callback(callback_id, "Unknown gate")
