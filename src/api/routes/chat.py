...
from api.schemas import ErrorResponse, ChatResponseModel
...
async def chat_with_supervisor(
    ...
    def _collect_turn_stream(agent_obj: object, messages: list[dict[str, str]], session_id: str):
        stream_iter = agent_obj.create_turn(messages, session_id, stream=True)  # type: ignore[attr-defined]
        final_response = None
    ...
    try:
        ...
        try:
            classification: ClassificationResult = await classify_request_async(
                async_client, body.message
            )
        except Exception:
            # Fallback to sync classification if async fails
            classification = await run_in_threadpool(classify_request, client, body.message)
        ...
        if not stream:
            ...
            try:
                ...
                return ChatResponse(response=text, run_id=run_id, actions_taken=[])
            finally:
            ...
            async def producer():
                try:
                    async for chunk in async_client.chat.completions.create(
                        ...
                        or request.app.state.get("default_model", None)
