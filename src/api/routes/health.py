...
from llama_clients import _get_circuit_breaker
from workflows.langgraph.graph import check_checkpointer_health
...
async def health_check(
    ...
    if deep:
        from llama_clients import get_async_client, list_models_async

        try:
            if settings.llama_stack_provider != "real":
                raise RuntimeError(f"LLAMA_STACK_PROVIDER={settings.llama_stack_provider}")
...
async def langgraph_health() -> JSONResponse:
    """Check LangGraph checkpointer connectivity."""
    ok = await check_checkpointer_health()
...
async def get_stuck_runs_endpoint(
    ...
    """
    from workflows.cleanup import get_stuck_runs
...
async def cleanup_stuck_runs_endpoint(
    ...
    """
    from workflows.cleanup import timeout_stuck_runs
