...
from services.render_provider import RenderJob, RenderProvider, RenderStatus
...
class LocalRemotionProvider:
    ...
    async def get_status(self, job_id: str) -> RenderJob:
        ...
        Polls the remotion-service /api/status/{job_id} endpoint to
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                resp = await client.get(
                    f"{self.service_url}/api/status/{job_id}",
                )
                resp.raise_for_status()
                data = resp.json()
            ...
            return RenderJob(
                job_id=job_id,
                status=status,
                artifact_url=data.get("artifact_url") or data.get("outputUrl"),
