  ...
  server.registerTool(
    ADAPTIVE_SEARCH_TOOL_NAME,
    {
      ...
      description: [
        ...
        '',
        '## When to Use prompts_search_adaptive',
        'Use ONLY when:',
        '- Query is ambiguous or uncertain',
        '- Initial prompt_search results inadequate (low utility)',
        '- Need iterative refinement with confidence scoring',
        '- Exploring unfamiliar territory',
        '',
        'Do NOT use when:',
        '- You have clear filters (persona/project) - use prompt_search instead',
        '- Need fast results - adaptive search is slower (3+ iterations)',
        '- Exact terms known - use prompt_search with keyword mode',
        '',
        '## How It Works',
        '1. Decides if retrieval needed (can return "no retrieval")',
        '2. Formulates optimized query from context',
        '3. Searches with current mode (vector → hybrid → keyword)',
        '4. Evaluates utility (0-1 score)',
        '5. Refines query if utility < threshold',
        '6. Repeats up to maxIterations or until utility met',
        '',
        '## Parameter Guidelines',
        'maxIterations: 1-5 (recommend 3 for most cases)',
        'utilityThreshold: 0-1 (recommend 0.75 for quality results)',
        'enableMultiHop: true to include graph expansion in iterations',
        '',
        '## Stopping Criteria',
        'Stops when:',
        '- Utility threshold reached',
        '- Max iterations reached',
        '- Utility improvement <0.05 between iterations',
        '- No refinement possible (query exhausted)',
      ].join('\n'),
