...
app.use((req, res, next) => {
  ...
  if (headerAuth === `Bearer ${API_SECRET}` || apiKey === API_SECRET) {
    return next();
  }
...
app.post('/api/render', async (req: Request, res: Response) => {
  ...
  if (hasCode && !SANDBOX_ENABLED) {
    res.status(400).json({ error: 'composition_code is disabled without sandboxing; use template mode' });
    return;
  }
